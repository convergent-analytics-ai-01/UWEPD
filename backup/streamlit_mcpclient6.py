...
def delete_thread(thread_id: str):
    ...
    try:
        if mem_path.exists():
            mem_path.unlink()
        # If the deleted thread was the active one, start a new chat
...
def handle_chat_selection():
    """Callback to load a chat when a radio button is selected."""
    selected_chat_message = st.session_state.selected_chat
    if selected_chat_message == "--- Start a new chat ---":
        start_new_chat()
        return
    
    # Find the thread_id that corresponds to the selected message
...
# --- Column 2: Chat History (UPDATED with scrollable list box) ---
with col2:
    ...
    # The delete button is now context-aware of the selection
    st.button(
        "🗑️ Delete Selected Chat",
        on_click=delete_selected_chat_callback,
        use_container_width=True,
        disabled=(st.session_state.selected_chat == placeholder) # Disable if no chat is selected
    )